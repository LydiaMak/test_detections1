...
import lsst.obs.swasp.printDict
import lsst.obs.swasp.swaspNullIsr
# Perform calibration?
...
# how large a region of the sky should be used for each background point
# 	Valid Range = [1,inf)
config.charImage.detection.tempLocalBackground.binSize=8
...
# specify the minimum psfFlux for good Psf Candidates
config.charImage.measurePsf.starSelector['objectSize'].fluxMin=0.0001
...
# specify the maximum psfFlux for good Psf Candidates (ignored if == 0)
config.charImage.measurePsf.starSelector['objectSize'].fluxMax=1000.0
...
# Mask planes with the corresponding limit on the fraction of masked pixels. Sources violating this limit will not be deblended.
config.calibrate.deblend.maskLimits={'SAT': 0.0}
...
# Maximum allowed shift of WCS, due to matching (pixel)
# 	Valid Range = [-inf,4000)
config.calibrate.astrometry.matcher.maxOffsetPix=600
...
# Maximum separation between reference objects and sources beyond which they will not be considered a match (arcsec)
# 	Valid Range = [0,inf)
config.calibrate.astrometry.matcher.maxMatchDistArcSec=7200.0
...
# Rotation angle allowed between sources and position reference objects (degrees)
# 	Valid Range = [-inf,6.0)
config.calibrate.astrometry.matcher.maxRotationDeg=5.0
...
# maximum median scatter of a WCS fit beyond which the fit fails (arcsec); be generous, as this is only intended to catch catastrophic failures
# 	Valid Range = [0,inf)
config.calibrate.astrometry.wcsFitter.maxScatterArcsec=2000.0
...
# how large a region of the sky should be used for each background point
# 	Valid Range = [1,inf)
config.calibrate.detection.tempLocalBackground.binSize=8
...
# Default reference catalog filter to use if filter not specified in exposure; if blank then filter must be specified in exposure
config.calibrate.refObjLoader.defaultFilter='v'
...
import lsst.obs.swasp.swaspNullIsr
config.isr.retarget(target=lsst.obs.swasp.swaspNullIsr.SwaspNullIsrTask, ConfigClass=lsst.obs.swasp.swaspNullIsr.SwaspNullIsrConfig)# Order of polynomial or to fit if overscan fit type is a polynomial, or number of spline knots if overscan fit type is a spline.
